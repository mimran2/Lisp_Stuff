...
(defun collatz(n)
   (cond 
        ( (= n 1) '(1))
        ( (= (mod n 2) 0) (append (list n) (collatz(/ n 2)))          )
        ( (= (mod n 2) 1) (append (list n) (collatz(+ (* n 3) 1)))    ) 
   )
)

(defun  list_report (xs)
  "write all to list"
  (list (float(/ (mean xs) (len xs))) (float(median xs)) (modeFinder xs) )  
...
(defun for_finder(uniqVals vals)
 "create a list of how often each unique val occurs"
(if (null uniqVals)
	'(0)
...
"for each value in nubs and counts if it is equal to max return that number"
 (if (null counts)
	'()
